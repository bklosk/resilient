...
def test_generate_and_validate_bbox():
    buffer_km = 1.0
    bbox = utils.BoundingBoxUtils.generate_bounding_box(
        40.0, -105.0, buffer_km=buffer_km
    )
...
def test_job_helpers():
    ...
    updated = app_module.update_job_status(
        job_id, status=app_module.JobStatus.COMPLETED
    )
    assert updated
